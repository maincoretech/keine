...
impl ApplicationHandler<WinitUserEvent> for WinitAppRunnerState {
    ...
    fn window_event(
        ...
        // `about_to_wait` unconditionally requests a redraw on macOS as a
        // workaround for bevyengine/bevy#17488. Counting the resulting
        // synthetic RedrawRequested event as fresh window input makes
        // Reactive mode immediately schedule another update and redraw,
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        #[cfg(target_os = "macos")]
        if self.should_update(self.update_mode) {
            // Asking macOS for a redraw on every `about_to_wait`, including
            // when Reactive mode has no pending work, creates a self-sustaining
            // RedrawRequested loop. Only bridge an actual event or elapsed
            // deadline into Bevy's update/render path.
            self.redraw_requested(event_loop);
        }
