...
- [x] v10 专用二进制存档：元数据前置、状态载荷分离、metadata/state 双 CRC32、只读前缀槽位检查、Program fingerprint、多粒子发射器状态、完整相机与句尾退格状态、`SavedState` 恢复边界、固定 golden、原子替换与独立 WebP 预览
...
- [x] 1.9.5–1.9.8 的段落样式、命名无限浮字/清理、系统 alert/confirm、距离×位置立绘布局
  与 sequence 逐帧立绘进入 typed core；Spine、Live2D 明确报 unsupported diagnostic
- [x] 1.9.9 `updateCharacter` 只更新在场立绘，原生支持表情/皮肤、距离×位置及可阻塞换位；
