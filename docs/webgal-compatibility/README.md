...
> **维护状态：冻结。** 本目录是现有兼容事实和迁移边界，不是功能路线图。项目不再
> 追求 WebGAL 语义对齐；现有行为只在安全、崩溃、数据损坏或 Kēne 自身改动造成明确
> 回归时维护。未来若决定移除兼容层，将作为单独的产品决策处理，而不是在普通维护中
> 静默删改。
...
`P+C+R` 不等于 `V`。当前仓库有语义、布局数学和渲染辅助逻辑的自动测试，但**没有截图快照、像素差分或其他自动视觉回归基线**。当前也不保留人工截图作为基线；页面、动画关键帧、尺寸与平台仍保持待验，详见 [visual-audit.md](visual-audit.md)。现有 `docs/acceptance/` 文件只是人工验收步骤，不是已经通过的证据。
